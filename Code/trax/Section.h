...
namespace trax{
	...
	struct Section{
		...
		enum class SpecialSections{
			...
			vignol_EB63T_rod,			///< vignol EB63T single rod in center
		...
		struct SectionPoint{
			...
			SectionPoint& operator+=( const spat::Vector2D<Length>& delta ){
				p += delta;
				return *this;
			}
		...
		/// \brief Moves all the points by delta.
		virtual void Shift( const spat::Vector2D<Length>& delta ) noexcept = 0;
